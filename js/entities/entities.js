...
game.tweenTime = function(from, to, speed) {
    var dx = from.x - to.x,
        dy = from.y - to.y,
...
game.PlayerEntity = me.ObjectEntity.extend({
    ...
    update: function() {
        ...
        if (this.collideType('finish')) {
            ...
            if (me.levelDirector.getCurrentLevelId() === '14') {
        ...
        var res = this.collideType('moving_block', false);
        if (res) {
    ...
    _stop: function() {
        ...
        this._stops = true;
        new me.Tween(this.pos).to(to, game.tweenTime(this.pos, to, 120)).onComplete(function() {
            this._stops = false;
        }.bind(this)).start();
...
game.MovingBlock = game.ActiveObject.extend({
    ...
    activate: function() {
        ...
        new me.Tween(this.pos).to(to, game.tweenTime(this.pos, to, 250))
            .onComplete(function() {
                collisions.clearTile(tile.col, tile.row);
...
game.Fan = me.ObjectEntity.extend({
    init: function(x, y, settings) {
        settings.image = 'fan';
        settings.spritewidth = 50;
        this.parent(x, y, settings);
